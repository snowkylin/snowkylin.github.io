...
在一般深度学习的讨论中，往往给出算法后就到此为止了，直接进入实验和结果部分。换言之，没有讨论“为什么可以这样做，为什么这样效果更好”的问题。然而深度信念网络却是有一定的理论依据的。接下来简要介绍深度信念网络中逐层训练的原理。
...
\begin{equation}
    P(v, h^{(1)}, h^{(2)}; \theta) = P(v\vert h^{(!)}; W^{(1)})P(h^{(1)}, h^{(2)}; W^{(2)})
\end{equation}
